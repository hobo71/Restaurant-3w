...

{
    ...
    public function find($userId){
        ...
        return $database->queryOne("SELECT * FROM User WHERE Id = ?",[ $userId ]);
...

    {
        ...
        $user = $database->queryOne("SELECT Id, LastName, FirstName, Email, Password FROM User WHERE Email = ?",[ $email ]);
    ...
    public function signUp($lastName, $firstName, $email, $password, $birthDate, $address, $city, $zipCode, $phone){
        ...
        $user = $database->queryOne("SELECT Id FROM User WHERE Email = ?", [ $email ]);
        ...
        $sql = "INSERT INTO User(LastName,FirstName,Email,Password,BirthDate,Address,City,ZipCode,Phone,CreationTimestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        ...
        [$lastName,$firstName,$email,$passwordHash,$birthDate,$address,$city,$zipCode,$phone]);
    ...
    public function updateLoginTimestamp($userId){
        ...
        $database->executeSql("UPDATE User SET LastLoginTimestamp = NOW() WHERE Id = ?",[ $userId ]);
