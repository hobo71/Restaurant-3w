...
class OrderModel
{
	...
	public function findOrderLines($orderId)
	{
        ...
        $sql = ('SELECT * FROM OrderLine INNER JOIN Meal ON Meal.Id = OrderLine.Meal_Id WHERE Order_Id = ?');
    ...
    public function validate($userId, array $basketItems)
    {
        ...
        ('INSERT INTO `Order` (User_Id,CreationTimestamp,TaxRate) VALUES (?, NOW(), 20.0)',[ $userId ]);
        ...
        $sql = ('UPDATE `Order`SET CompleteTimestamp = NOW(),TotalAmount = ?,TaxAmount = ? * TaxRate / 100 WHERE Id = ?');

        $database->executeSql($sql, [ $totalAmount, $totalAmount, $orderId ]);
